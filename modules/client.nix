  ...
  serverAliases = builtins.listToAttrs (builtins.concatLists (map (server: [
      {
        ...
        value = "NIX_SSHOPTS='-o ControlMaster=no' nixos-rebuild switch --flake ${serversFlake}#${server.nixosAttr} --target-host ${server.sshAlias} --build-host ${server.sshAlias} --impure --refresh";
      }
      {
        name = "update-${server.name}";
        value = "NIX_SSHOPTS='-o ControlMaster=no' nixos-rebuild switch --flake ${serversFlake}#${server.nixosAttr} --override-input k3s-cluster github:Clusterforgers/k3s-cluster --target-host ${server.sshAlias} --build-host ${server.sshAlias} --impure --refresh";
      }
      {
